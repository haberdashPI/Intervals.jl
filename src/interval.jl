"""
    Interval{T, L <: Boundedness, U <: Boundedness}
...
"""
struct Interval{T, L <: Boundedness, U <: Boundedness} <: AbstractInterval{T,L,U}
    lowerbound::T
    upperbound::T
...
# Bound constructors
function Interval{T}(lower::LowerBound{T,L}, upper::UpperBound{T,U}) where {T,L,U}
    Interval{T,L,U}(endpoint(lower), endpoint(upper))
end

function Interval{T}(lower::LowerBound, upper::UpperBound) where T
    Interval{T, bound_type(lower), bound_type(upper)}(endpoint(lower), endpoint(upper))
end

function Interval(lower::LowerBound{S}, upper::UpperBound{T}) where {S,T}
...
function overlaps(a::AbstractInterval, b::AbstractInterval)
    lower = max(LowerBound(a), LowerBound(b))
    upper = min(UpperBound(a), UpperBound(b))
...
function Base.merge(a::AbstractInterval, b::AbstractInterval)
    ...
    lower = min(LowerBound(a), LowerBound(b))
    upper = max(UpperBound(a), UpperBound(b))
    return Interval(lower, upper)
end
...
function _round(
    f::RoundingFunctionTypes, interval::Interval{T,L,U}, on::Val{:lower}, args...
) where {T, L <: Unbounded, U <: Boundedness}
    return interval
end
...
function _round(
    f::RoundingFunctionTypes, interval::Interval{T,L,U}, on::Val{:upper}, args...
) where {T, L <: Boundedness, U <: Unbounded}
    return interval
end
